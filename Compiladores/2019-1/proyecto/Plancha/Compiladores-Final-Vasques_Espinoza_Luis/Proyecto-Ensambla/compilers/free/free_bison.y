...
%token ID NUM ENTER STRING COM IF IFELSE WHILE FOR DEF /* - */ TRUEASSIGN ASSIGN SUM RES MUL DIV EVAL EQ NEQ LES MOR LEQ MEQ ITERLIMIT AND OR POW /* - */ PROG VARS ELSE OTHERCASE DESDE HASTA AUMENTANDO

%%
superroot: PROG ID '{' root '}';
...
exp:	VARS multipleassigns {$$=$1;}
	|	assign {$$=$1;}
	|	call	{$$=$1;};
	
multipleassigns:	assign COM multipleassigns
				|	assign;
...
//ifStructure:		IF statement {evalFlag=evalCondition($2);} '{' lines '}' {evalFlag=1;};
ifElseStructure:	IF statement {evalFlag=evalCondition($2);} '{'
	{
		elseFlag = (evalFlag+1)%2;
		otherCaseFlag = (evalFlag+1)%2;
	} lines '}' ElseStructure;

ElseStructure: ELSE statement
			{
				if(!elseFlag){evalFlag = 0;}
				else{
					evalFlag=evalCondition($2);
					if(evalFlag){
						otherCaseFlag=0;
					}else{
						otherCaseFlag=1;
			...
			} '{' lines '}' DefaultStructure;

DefaultStructure: OTHERCASE {evalFlag=otherCaseFlag;} '{' lines '}' {evalFlag=1;};
...
forStructure:		'[' FOR '(' DESDE assign ';' {$$=cx+1;} HASTA statement {$$=cx;} {evalFlag=0;}';' {$$=cx;} AUMENTANDO assign {$$=cx;} ')' {evalFlag=0;} lines
					{
						int A1=$7; //primer $$
						int A2=$10; //segundo $$

						int B1=$13+1; //tercer $$ + 1
						int B2=$16; // cuarto $$
						...
						evalFlag=1;
						while(evalCondition($9)) // statement
						{
							evalLoop(A1,CX);
						}
					} ']';
