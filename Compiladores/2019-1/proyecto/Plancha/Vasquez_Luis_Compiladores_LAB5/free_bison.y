...
%token ID NUM ENTER STRING COM IF IFELSE WHILE FOR DEF /* - */ TRUEASSIGN ASSIGN SUM RES MUL DIV EVAL EQ NEQ LES MOR LEQ MEQ ITERLIMIT AND OR /* - */ ALG INIALG FINALG THEN IFSTART IFEND

%%
root:	/* empty */
	|	ALG ID INIALG root lines FINALG;
...
ifStructure:		IF statement {evalFlag=evalCondition($2);} THEN IFSTART lines IFEND {evalFlag=1;};
