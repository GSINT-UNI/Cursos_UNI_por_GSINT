...
%}
%token DESDE HASTA HACER PROGRAMA ID INICIO FIN NUM VARIABLE SI SINO ASIGNAR SUMAR RESTAR MULTIPLICAR DIVIDIR MAYOR MENOR IGUAL MAYOR_IGUAL MENOR_IGUAL IF ENTONCES SALTARF1 SALTARF2 IMPRIMIR PARENTESIS BUCLE BUCLE_1 BUCLE_2 SALTARF3 SQRT RAIZ INICIO_IF FIN_IF TERMINAL

%%

s : PROGRAMA ID INICIO listaInstr FIN '.' ;

listaInstr: asignacion listaInstr | bloque_if listaInstr |imprimir listaInstr |;


bloque_if: SI condicion {printf("generando En el bloque_if...\n");generarCodigo(SALTARF1,$2,'-','-');printf("$2 es %d\n",$2);$$=cx;} ENTONCES INICIO_IF listaInstr FIN_IF {generarCodigo(SALTARF2,'?','-','-');printf("Saltarf2 generando en bloque_if, ? es %d\n",'?');$$=cx;} {TCodigo[$3].a2=cx+1;printf("Valor de $2: %d Valor de cx: %d TCodigo[$3].a2 es %f \n",$2,cx,(double)cx+1);} otro {printf("$8 es %d, TCodigo[$8].a1 sera %d\n",$8,cx+1);;TCodigo[$8].a1=cx+1;};
