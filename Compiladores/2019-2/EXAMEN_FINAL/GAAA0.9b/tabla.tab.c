...
#line 218 "tabla.y"
    {
						...
						if(VERBOSE) printf("Valor de $5: %d Valor de cx: %d TCodigo[$<itype>5].a2 es %f \n",(yyvsp[-4].itype),cx,(double)cx+1);
					}
...
const int MAX_NODE_N = 1e4;

typedef pair<int, map<char, int> > Node;

int lastNode = 1;
int lastId = 0;

Node NODES[MAX_NODE_N];
...
int trie(string s) {
    ...
    for(int i = 0; i < s.size(); i++) {
        char c = s[i];
        if( NODES[x].second.insert({c, -1}).second ) {
            x = NODES[x].second[c] = lastNode++;
        } else {
            x = NODES[x].second[c];
        }
    }
...
int localizaSimb(char *nom,int tok){
  ...
  if(prev == nSim) return i;

  // new node

  strcpy(TablaSim[i].nombre,nom);
  TablaSim[i].token=tok;
  if (tok==ID) {  /*
  	printf("Parece que es un ID .... \n");
  	cout << "Tenemos... " << tok << "\n"; 
	cout << "TablaSim[i].nombre = **  " << TablaSim[i].nombre << " ** esto\n";*/
	TablaSim[i].valor=0.0;
	}
  if (tok==NUM) {  
  	sscanf(nom,"%lf",&TablaSim[i].valor);
	}
  return i;
}
