...
%}
%token DESDE HASTA HACER PROGRAMA ID INICIO FIN NUM VARIABLE SI SINO ASIGNAR SUMAR RESTAR MULTIPLICAR DIVIDIR MAYOR MENOR IGUAL MAYOR_IGUAL MENOR_IGUAL IF ENTONCES SALTARF1 SALTARF2 IMPRIMIR PARENTESIS BUCLE BUCLE_1 BUCLE_2 SALTARF3 SALTARF4 WHILE MIENTRAS
%%

s : PROGRAMA ID ';'  INICIO listaInstr FIN '.' ;

listaInstr:bucle_while listaInstr| asignacion listaInstr | bloque_if listaInstr |bucle_for listaInstr|imprimir listaInstr |;
...
number:NUM {$$=localizaSimb(lexema,NUM);}|ID{$$=localizaSimb(lexema,ID);}|'-' NUM {char str[255];strcpy(str, "-");strcat(str, lexema);$$=localizaSimb(str, NUM);}
;
...
bucle_while: MIENTRAS '(' condicion ')' HACER {generarCodigo(SALTARF4,$3,'-','-');posicion_while=cx;$$=cx;} '{' listaInstr '}' {generarCodigo(WHILE,'?','-','-');$$=cx;}{printf("$6 es %d TCodigo[$6].a2 sera %d\n",$6,cx);TCodigo[$6].a2=cx;}{printf("$10 es %d , TCodigo[$10].a1 sera %d \n",$10,cx+1);TCodigo[$10].a1=cx+1;TCodigo[$10].a2=posicion_while;};
