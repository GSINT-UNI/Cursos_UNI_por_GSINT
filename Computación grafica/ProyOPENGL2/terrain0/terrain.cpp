...
void loadTextures() {
    ...
    int imgHeight;  // Alto.
    FREE_IMAGE_FORMAT format = FreeImage_GetFIFFromFilename("pastoroca.jpg");
    if (format == FIF_UNKNOWN) {
       printf("Archivo de Imagen desconocido \n");
       }
    FIBITMAP* bitmap = FreeImage_Load(format, "pastoroca.jpg", 0);  //Leer Imagen.
    if (!bitmap) {
		...
		for (j = 0;j < terrainGridWidth; j=j+2) {
			...
			glVertex3f(
				...
				startL - i + zOffset);		
                        ...
                        printf("z1:=%f,\nz2:=%f,\nz3:=%f,\nz4:=%f\n",startL - (i+1) + zOffset,startL - i + zOffset,startL - (i+1) + zOffset,startL - i + zOffset);
		}
		glEnd();
	}
	glEndList();
