...
// For Bezier curve
const int numcontrolpts = 8;  // Number of control points
...
/*
   ...
   glBegin(GL_LINE_STRIP);
      for (int i=0; i<=numdrawsegs; ++i)
         glVertex3f(controlpts[i][0],controlpts[i][1],controlpts[i][2]);
   glEnd();
*/	
   ...
   glBegin(GL_LINE_STRIP);
      for (int i=0; i<=numdrawsegs/2; ++i)
         glEvalCoord1d(GLdouble(i)/numdrawsegs);
   glEnd();
   
   glBegin(GL_LINE_STRIP);
      for (int i=4; i<=numdrawsegs; ++i)
         glEvalCoord1d(GLdouble(i)/numdrawsegs);
   glEnd();
...
void keyboard(unsigned char key, int x, int y)
{
   ...
   case 'x':
   case 'X':
		//controlpts[3][0]=controlpts[3][0]+1;
		//glutPostRedisplay();
		//break;
		//dx4=dx4+0.1;
		//dz4=dz4+0.0;                              
   case 'y':
   case 'Y':
		//controlpts[3][1]=controlpts[3][1]+0.1;
		//glutPostRedisplay();
   		//dy4=dy4+0.1;
		//dz4+=0.0;                              
		break;
