...
class fs:
    ...
    def createLink(self, tArgumentet, newfile, parent):
        ...
        tinum = self.nameToInum[tArgumentet]
    ...
    def doLink(self):
        ...
        tArgumentet = self.files[int(random.random() * len(self.files))]
        ...
        dprint('try createLink(%s %s %s)' % (tArgumentet, nfile, parent))
        inum = self.createLink(tArgumentet, nfile, parent)
        if inum >= 0:
            self.files.append(fullName)
            self.nameToInum[fullName] = inum
            if printOps:
                print('link("%s", "%s");' % (tArgumentet, fullName))
            return 0
        return -1
...
print('Argument numInodes',   options.numInodes)
print('Argument numData',     options.numData)
print('Argument numRequests', options.numRequests)
print('Argument reverse',     options.reverse)
print('Argument printFinal',  options.printFinal)
print('')
