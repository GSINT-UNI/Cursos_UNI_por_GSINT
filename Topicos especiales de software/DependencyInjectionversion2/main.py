...
from contenedor import Crafts,Powers,Enemies
...
class Set_Enemy():
    def __init__(self):
        ...
        self.enemyFire = [Enemies.fire(self) for i in range(self.nro_enemies_for_type)]
        self.enemyIce = [Enemies.ice(self) for i in range(self.nro_enemies_for_type)]
        self.enemyPoison = [Enemies.poison(self) for i in range(self.nro_enemies_for_type)]
...
class App_Game():
    def __init__(self):
        ...
        # Music of Background
        mixer.music.load('sound/background.ogg')
        mixer.music.play(-1)
        # Title and icon
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.KEYDOWN:
                    ...
                    if event.key == pygame.K_z:  
                        if self.player.bullet.state is "ready": 
                            if self.change == 1:
                                self.player.bullet = Powers.ice(self)
                                self.change = (self.change+1)%4
                            elif self.change == 2:
                                self.player.bullet = Powers.poison(self)
                                self.change = (self.change+1)%3
                            elif self.change == 0:
                                self.player.bullet = Powers.fire(self)
                                self.change = (self.change+1)%3    
