...
to setup
  ...
  set selected-car one-of ambulances
...
to go
  ...
  ask ambulances [
    let pche patch (pxcor - 1) pycor
    dar-paso
    ask pche [
      set chemical chemical + 5
    ]
  ]
...
to select-car
  ...
  if mouse-down? [
    ...
    if any? ambulances-on patch mx my [
      ;ask selected-car [ set shape "ambulance" ]
      ask selected-ambulance [ set color car-color ]
      set selected-ambulance one-of ambulances-on patch mx my
      ask selected-ambulance [ set color red ]
      display
    ]
  ]
end
...
to look-for-food  ;; turtle procedure
  ...
  if (chemical >= 0.05) and (chemical < 2) and (target-lane = ycor)
  [ uphill-chemical ]
end
...
to uphill-chemical  ;; turtle procedure
  ...
  [ ifelse scent-right > scent-left
    ...
    let min-dist min map [ y -> abs (y - ycor) ] other-lanes
    let closest-lanes filter [ y -> abs (y - ycor) = min-dist ] other-lanes
    set target-lane min closest-lanes
    set patience max-patience
    ] ]
    [ let other-lanes remove ycor lanes
    if not empty? other-lanes [
    let min-dist min map [ y -> abs (y - ycor) ] other-lanes
    let closest-lanes filter [ y -> abs (y - ycor) = min-dist ] other-lanes
    set target-lane max closest-lanes
    set patience max-patience
    ] ] ]
end
...
@#$#@#$#@
## CAMBIOS REALIZADOS  PARA EL FINAL
 - Agregada una "feromona" detras de la ambulancia para general que los autos traten de posicionarse detras de él para ir lo mas rapido posible.
## CÓDIGO
 - Se creo una clase especifica para contener las fermonas llamada patches-own
 - En la función "go" se especifica que el quimico se crea en el patch detras de la ambulancia.
 - la función "uphill-chemical" es el comportamiento de los carros que les permite "oler" la feromona y decidir a que carril deben moverse para estar mas cerca de la feromona, en este caso ese punto es exactamente detras de la ambulancia.
