...
to setup
  ...
  set-default-shape ambulances "ambulance"
  draw-road
  ...
  create-or-remove-ambulances
  set selected-car one-of cars
  ;ask selected-car [ set shape "ambulance" ]
  ask selected-car [ set color red ]
  ;ask selected-car [ set size 2 ]
  reset-ticks
end
...
to create-or-remove-cars
  ...
  create-cars (number-of-cars - count cars) [
    ...
    set heading 90
    set top-speed 1.2 + random-float 0.5
    set speed 0.5
...
to create-or-remove-trucks
  ...
  create-trucks (number-of-trucks - count trucks) [
    set color one-of [ lime brown gray ] + 1.5 + random-float 1.0
    move-to one-of free-truck road-patches
    ;patch (-20 + random-float 40) (2)
    set target-lane pycor
    set heading 90
    set top-speed 1 + random-float 0.5
    set speed 0.3
    set size 2
    set patience random max-patience
  ]
  ...
  if count trucks > number-of-trucks [
    ...
    ask n-of n [ other trucks ] of selected-car [ die ]
  ]

end

to create-or-remove-ambulances
  let road-patches patches with [ member? pycor lanes ]
  if number-of-cars > count road-patches [
    set number-of-cars count road-patches
  ]
  create-ambulances (number-of-ambulances - count ambulances) [
    set color white
    move-to one-of free road-patches
    ;patch (-20 + random-float 40) (2)
    set target-lane pycor
    set heading 90
    set top-speed 1.7 + random-float 0.2
    set speed 0.7
    set size 2
    set patience random max-patience
  ]

  if count ambulances > number-of-ambulances [
    let n count ambulances - number-of-ambulances
    ask n-of n [ other ambulances ] of selected-car [ die ]
  ]
...
to dar-paso
  let lista-obstaculos other turtles in-cone 4 180 with [ y-distance = 0]
  let obstaculo min-one-of lista-obstaculos [ distance myself ]
  if obstaculo != nobody [
      ask obstaculo [ choose-new-lane ]
...
to move-forward ; car procedure
  ...
  let blocking-turtles other turtles in-cone (1 + speed) 180 with [ y-distance <= 1 ]
  let blocking-turtle min-one-of blocking-turtles [ distance myself ]
  if blocking-turtle != nobody [
    ; match the speed of the car ahead of you and then slow
    ; down so you are driving a bit slower than that car.
    set speed [ speed ] of blocking-turtle
    slow-down-car
  ]
  forward speed
end
...
to choose-new-lane ; car procedure
  ...
  if is-car? self or is-ambulance? self [
...
to move-to-target-lane ; car procedure
  ...
  let blocking-turtles other turtles in-cone (1 + abs (ycor - target-lane)) 180 with [ x-distance <= 1 ]
  let blocking-turtle min-one-of blocking-turtles [ distance myself ]
  ifelse blocking-turtle = nobody [
    forward 0.2
    set ycor precision ycor 1 ; to avoid floating point errors
  ] [
    ; slow down if the car blocking us is behind, otherwise speed up
    ifelse towards blocking-turtle <= 180 [ slow-down-car ] [ speed-up-car ]
  ]
end
...
@#$#@#$#@
## CAMBIOS REALIZADOS 
 - Se agregaron tres clases para diferenciar entre los vehículos carro, trailer y ambulancia
 - Se crearon dos funciones extras para crear y dar caracteristicas a los trailer y las ambulancias
 - Se agrego el comportamiento de seder el paso a la ambulancia en la funcion "dar paso"
 - Se hace una diferenciacion entre los carriles permitidos solo para los trailers mediante las variables globales lanes y lanes-truck donde lanes-truck son los carriles permitidos para los trailes y lanes el resto de carriles
 - En choose-new-lane se agrego un comportamiento que diferencia entre trailers y no trailers para que cada vehiculo seleccione un carril que le este permitido

## OBSERVACIONES
 - Se observa que cuando dos carros se acercan mucho ambos disminuyen la velocidad.
 - Se observa que si un carro decide cambiar de carril pero no puede, empeora el tráfico.
 - Se observa que entre mayor sea la paciencia de los autos mas fluido es el tráfico.

## IDEAS PARA MEJORAR EL MODELO 

 - Para hacer el modelo mas cercano a la realidad prodiamos implementar una "Parada de pasajeros donde vehiculos tipo "bus" o "taxi" detengan el trafico un tiempo mientras esperan pasajeros.
 - Tambien se podria agregar una cruce direccional para mostrar los fenomenos de embotellamiento.
 - Agregar doble sentido a la via de tránsito.

## IDEAS PARA MEJORAR EL TRÁNSITO
 - Tratar de que los vehículos mantengan su carril lo máximo posible
 - Un sistema de semáforo inteligente que de mas tiempo en verde de acuerdo a la cantidad de carros que esperan para pasar
 - Designar la mayor cantidad de carriles a los vehiculos que generan mayor tráfico en las vias.
 - Evitar que un carro se quede parado mucho tiempo recogiendo pasajeros durante una luz verde
